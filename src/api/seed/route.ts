...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  try {
    ...
    if (!salesChannels.length) {
      return res.status(400).json({ error: "No sales channel found. Call /setup first." });
    }
    ...
    if (!regions.length) {
      const { result } = await createRegionsWorkflow(req.scope).run({
        input: {
          regions: [{ name: "United States", currency_code: "usd", countries: ["us"], payment_providers: ["pp_system_default"] }],
        },
      });
      regions = result;
    }
    ...
    res.json({ message: "Products seeded successfully", count: barDiamondProducts.length, categories: categoryNames });
  } catch (error) {
